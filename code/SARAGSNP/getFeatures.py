...
import os
from SARAGSNP.Featurizer import *
import pickle
import time
...
def get_smiles_array(smilesList, feature_dicts):
    '''
    将 SMILES 列表转为神经网络输入张量(图结构)
    x_mask.append每个分子的原子掩码，用于处理batch 中 padding问题长度为 max_num_atoms_in_batch，1 表示有效原子，0 表示补齐用的 placeholder
    x_atom.append每个分子的原子特征向量集合，通常是 [num_atoms, num_atom_features] 的 2D 数组每一行代表一个原子的数值化描述（例如类型、杂化、是否在环上、电荷等）
    x_bonds.append每个分子的键特征向量集合，即 [num_bonds, num_bond_features]每条键（边）可能包括类型（单、双等）、是否在环上、是否是共轭键等信息
    x_atom_index.append每个分子的原子邻接信息，用来标识每个原子的邻居原子索引，例如 [[1, 2], [0, 2], [0, 1]]。
    x_bond_index.append每条键的邻接键索引，例如 [[1], [0, 2], [1]]，用来捕捉图结构中更复杂的邻接关系
    '''
    x_mask = []
    x_atom = []
    x_bonds = []
    ...
    # 返回值每个都是 numpy.ndarray，形状为 [batch_size, ...]，这些就是 GNN 的输入
