...
import os
from SARAGSNP.Featurizer_aromaticity_rm import *
import pickle
import time
...
def get_smiles_array(smilesList, feature_dicts):
    # 检查是否包含所有必要的键
    required_keys = [
        'smiles_to_atom_mask', 'smiles_to_atom_info', 'smiles_to_bond_info',
        'smiles_to_atom_neighbors', 'smiles_to_bond_neighbors', 'smiles_to_rdkit_list'
    ]
    for key in required_keys:
        if key not in feature_dicts:
            raise KeyError(f"Missing required key in feature_dicts: {key}")
    ...
    for smiles in smilesList:
        # 确保 SMILES 是标准格式
        standardized_smiles = Chem.MolToSmiles(Chem.MolFromSmiles(smiles))

        if standardized_smiles not in feature_dicts['smiles_to_atom_mask']:
            print(f"Warning: {smiles} (standardized: {standardized_smiles}) not found in feature_dicts['smiles_to_atom_mask']")
        
        x_mask.append(feature_dicts['smiles_to_atom_mask'][standardized_smiles])
        x_atom.append(feature_dicts['smiles_to_atom_info'][standardized_smiles])
        x_bonds.append(feature_dicts['smiles_to_bond_info'][standardized_smiles])
        x_atom_index.append(feature_dicts['smiles_to_atom_neighbors'][standardized_smiles])
        x_bond_index.append(feature_dicts['smiles_to_bond_neighbors'][standardized_smiles])
